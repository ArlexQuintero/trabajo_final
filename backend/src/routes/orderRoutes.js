...
router.post('/', verifyToken, async (req, res) => {
  const { total, items } = req.body; // items: [{ producto_id, cantidad, precio_unitario }]
  const usuario_id = req.user.id; 

  try {
    if (!items || items.length === 0) {
      return res.status(400).json({ error: 'No puedes generar un pedido sin productos.' });
    }
    ...
    res.status(201).json({ message: '¡Pedido simulado con éxito! 🐶🛒', pedido_id });
  } catch (error) {
...
router.put('/:id/status', verifyToken, isAdmin, async (req, res) => {
  const { status } = req.body; // Ej: 'enviado', 'entregado', 'cancelado'
  try {
